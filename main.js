'use strict';
{
  ...
  function draw() {
    ...
    img1.addEventListener('load', () => {
      ...
      //塗りのパターンとして指定
      const pattern = ctx.createPattern(img1, 'repeat');
    ...
    //画像(切出し)
    let ctx2 = canvas[7].getContext('2d');
    ...
    //描画サイズ変更
    const ctx3 = canvas[8].getContext('2d');
    ...
    //描画領域の移動回転
    const ctx4 = canvas[9].getContext('2d');
    ...
    //描画設定の保存・復元
    const ctx5 = canvas[10].getContext('2d');
